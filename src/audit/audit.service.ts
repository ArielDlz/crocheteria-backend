...
import { PermissionAudit, PermissionAuditDocument, PermissionAuditAction } from './schemas/permission-audit.schema';
import { LoginAudit, LoginAuditDocument, LoginAuditAction } from './schemas/login-audit.schema';
...
export class AuditService {
  constructor(
    @InjectModel(PermissionAudit.name) private permissionAuditModel: Model<PermissionAuditDocument>,
    @InjectModel(LoginAudit.name) private loginAuditModel: Model<LoginAuditDocument>,
  ) {}
  ...
  async getPermissionAuditByUser(userId: string, limit = 50): Promise<PermissionAuditDocument[]> {
  ...
  async getPermissionAuditByAdmin(adminId: string, limit = 50): Promise<PermissionAuditDocument[]> {
    return this.permissionAuditModel
  ...
  async deletePermissionAuditsBefore(date: Date): Promise<number> {
    const result = await this.permissionAuditModel.deleteMany({
      createdAt: { $lt: date },
    }).exec();
  ...
  async getLoginAuditByUser(userId: string, limit = 50): Promise<LoginAuditDocument[]> {
  ...
  async getLoginAuditByEmail(email: string, limit = 50): Promise<LoginAuditDocument[]> {
    return this.loginAuditModel
      .find({ email })
      .sort({ createdAt: -1 })
      .limit(limit)
  ...
  async getFailedLoginAttempts(email: string, since: Date): Promise<number> {
    return this.loginAuditModel.countDocuments({
      email,
  ...
  async deleteLoginAuditsBefore(date: Date): Promise<number> {
    const result = await this.loginAuditModel.deleteMany({
      createdAt: { $lt: date },
    }).exec();
    return result.deletedCount;
  }
